...
inline long long get_index_II(const int t, const int x, const int y, const int z, const int T, const int L, const int mu)
{
  int tt;
  if (!open_boundary_conditions) tt= (t+T)%T;
  if (open_boundary_conditions) {
  ...
  long long ix = ((tt*L+xx)*L+yy)*L+zz;
  return (4*ix+mu) * 8;
}



inline long long ggi(const int ix, const int mu)
{
  if (ix==-1) return -1;
  else return ((long long)4*ix+mu) * 8;
}
